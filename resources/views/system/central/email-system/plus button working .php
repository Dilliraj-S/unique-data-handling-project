            ...
            $(document).on('click', '.send-immediately', function() {
                ...
                $sequenceForms.each(function(index) {
                    ...
                    const sequenceData = {
                        ...
                        template_id: $form.find(`#sequence-template-${setId}-${formTabIndex}`).val(),
                        audience_id: formTabIndex === 1 ? $form.find(`#sequence-audience-${setId}-${formTabIndex}`).val() : null,
                        from_emails: $form.find(`#sequence-from-email-${setId}-${formTabIndex}`).val() || [],
                        time_gap: formTabIndex === 1 ? parseInt($form.find(`#sequence-time-gap-${setId}-${formTabIndex}`).val()) || null : null,
                        batch_size: formTabIndex === 1 ? parseInt($form.find(`#sequence-batch-size-${setId}-${formTabIndex}`).val()) || null : null,
                        wait_time: parseInt($form.find(`#sequence-wait-time-${setId}-${formTabIndex}`).val()) || 0,
                        wait_unit: $form.find(`#sequence-wait-unit-${setId}-${formTabIndex}`).val() || 'minutes',
                        categories: formTabIndex > 1 ? ($form.find(`#sequence-report-filters-${setId}-${formTabIndex}`).val() || []) : []
                    };
                    sequences.push(sequenceData);
                });
                if (!allValid) {
                ...
                axios.post('/drift/save-sequences', {
                    ...
                    .then(response => {
                        ...
                        sequences.forEach((sequence, index) => {
                            const formTabIndex = index + 1;
                            if (!sequence.sequence_id || sequence.sequence_id.startsWith('new-')) {
                                const newSequenceId = response.data.sequences?.[index]?.id || sequence.sequence_id;
                                if (newSequenceId && newSequenceId !== sequence.sequence_id) {
                                    sequenceIdsMap[setId][formTabIndex] = String(newSequenceId);
                                    $(`#sequence-${setId}-${formTabIndex}`).attr('data-sequence', newSequenceId);
                                    $(`#sequence-${setId}-${formTabIndex}-form input[name="sequence_id"]`).val(newSequenceId);
                                    $(`.delete-sequence-tab[data-tab-index="${formTabIndex}"][data-set="${setId}"]`).attr('data-sequence', newSequenceId);
                                    if (formTabIndex === 1) {
                        ...
                        const firstSequenceId = $(`#sequence-${setId}-1-form input[name="sequence_id"]`).val();
                        const $firstForm = $(`#sequence-${setId}-1-form`);
                        ...
                        data.forEach(item => {
                            if (item.name === 'from_emails[]') {
                                ...
                                if (item.value !== '__select_all__') formData['from_emails'].push(item.value);
                        ...
                        axios.post('/api/drift/send', formData)
                            .then(response => {
                                ...
                                setTimeout(() => {
                                    ...
                                    $(`.view-reports[data-sequence="${newSequenceId}"]`).text('Hide Reports');
                            ...
                            .catch(error => {
                                ...
                                alert('Failed to send sequence: ' + (error.response?.data?.details || 'Unknown error'));
                                $btn.prop('disabled', false).find('.loading-spinner').remove();
                            });
                    })
                    .catch(error => {
                        console.error('Failed to save sequences:', error);
                        alert('Failed to save sequences: ' + (error.response?.data?.error || 'Unknown error'));
                        $btn.prop('disabled', false).find('.loading-spinner').remove();
                        $saveBtn.prop('disabled', false).find('.spinner-border').remove();
                    });
            });
                ...
                axios.post('/drift/save-sequences', {
                    ...
                    .then(response => {
                        ...
                        sequences.forEach((sequenceData, index) => {
                            const formTabIndex = index + 1;
                            if (!sequenceData.sequence_id || sequenceData.sequence_id.startsWith('new-')) {
                                const newSequenceId = response.data.sequences?.[index]?.id || sequenceData.sequence_id;
                                if (newSequenceId && newSequenceId !== sequenceData.sequence_id) {
                                    sequenceIdsMap[setId][formTabIndex] = String(newSequenceId);
                                    $(`#sequence-${setId}-${formTabIndex}`).attr('data-sequence', newSequenceId);
                                    $(`#sequence-${setId}-${formTabIndex}-form input[name="sequence_id"]`).val(newSequenceId);
                                    $(`.delete-sequence-tab[data-tab-index="${formTabIndex}"][data-set="${setId}"]`).attr('data-sequence', newSequenceId);
...
.then(response => {
    ...
    createSequenceTab(sequenceCount, newSequenceId, response.data.sequence.name,
        setId, `sequenceTabs-${setId}`, `sequenceTabContent-${setId}`, sequenceCount);
    fetchInitialData();
