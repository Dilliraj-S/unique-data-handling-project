...
/**
     ...
     * @param string $system Database system name
     * @param string $table Table name
     * @param array $params Parameters: ['columns' => array|string, 'joins' => array, 'where' => array, 'sort' => array, 'groupBy' => array, 'custom' => array, 'search' => string, 'order_by' => array, 'limit' => int, 'offset' => int, 'initial_load' => bool]
     * @param string|null $limit Optional limit for pagination (deprecated; use $params['limit'])
     * @return array Response: ['status' => bool, 'message' => string, 'data' => array]
     */
    public function get(string $system, string $table, array $params = [], string $limit = ''): array
    {
        try {
            ...
            if (!empty($params['where'])) {
                $query->where($this->buildWhereClause($params['where'], $table));
            }

            // Apply search if provided (LIKE on display column, e.g., 'name')
            if (!empty($params['search'])) {
                $columns = $this->getDisplayColumns($params, $table);
                $valueColumn = reset($columns) ?: 'name'; // Use value column from token or default to 'name'
                $query->where($valueColumn, 'LIKE', '%' . $params['search'] . '%');
            } elseif (isset($params['initial_load']) && $params['initial_load']) {
                // Apply initial load (small set of records, ordered by display column)
                $columns = $this->getDisplayColumns($params, $table);
                $valueColumn = reset($columns) ?: 'name'; // Use value column from token or default to 'name'
                $query->orderBy($valueColumn, 'ASC');
                $params['limit'] = $params['limit'] ?? 100; // Default to 10 for initial load
                $params['offset'] = $params['offset'] ?? 0;
            }

            // Apply ordering if provided (e.g., ['name' => 'ASC'])
    ...
    /**
     * Counts total records matching the conditions (for pagination).
     *
     * @param string $system Database system name
     * @param string $table Table name
     * @param array $params Parameters: ['where' => array, 'search' => string, 'joins' => array]
     * @return int Total count
     */
    public function count(string $system, string $table, array $params = []): int
    {
        try {
            $system = $system === 'open' ? 'central' : $system;

            // Build base query (similar to get, but for count)
            ...
            if (!empty($params['joins'])) {
                foreach ($params['joins'] as $join) {
                    if (isset($join['type'], $join['table'], $join['on']) && is_array($join['on'])) {
                        ...
                        if (in_array($method, ['innerJoin', 'leftJoin', 'rightJoin', 'crossJoin'])) {
                            $query->$method($join['table'], $join['on'][0], '=', $join['on'][1]);
                        }
                    }
                }
            }
            ...
            Developer::error('Error counting records', [
                'system' => $system,
                'table' => $table,
                'error' => $e->getMessage(),
            ]);
    ...
    public function filter(string $table, array $params): array
    {
        ...
        try {
            ...
            $liCompanyIdCountQueries = [
                'total' => [
                'sql' => $totalQuery->clone()
                    ->join('sun.master_accounts as ma', 'ma.li_smtp', '=', 'sun.master_leads.li_smtp')
                    ->selectRaw('COUNT(DISTINCT ma.li_smtp) as count')
                    ->toSql(),
                'bindings' => $totalQuery->getBindings(),
            ],

               'filtered' => [
                'sql' => $countQuery->clone()
                    ->join('sun.master_accounts as ma', 'ma.li_smtp', '=', 'sun.master_leads.li_smtp')
                    ->selectRaw('COUNT(DISTINCT master_leads.li_smtp) as count')
                    ->toSql(),
    ...
    /**
         * Gets display columns from token parameters or defaults to 'name'.
         *
         * @param array $params Parameters containing token or columns info
         * @param string $table Table name
         * @return array Array of column names
         */
        private function getDisplayColumns(array $params, string $table): array
        {
            // If token is provided, try to resolve columns from it
            if (!empty($params['token'])) {
                try {
                    $reqSet = Skeleton::resolveToken($params['token']);
                    if (isset($reqSet['value']) && is_array($reqSet['value'])) {
                        return array_values($reqSet['value']);
                    }
                } catch (Exception $e) {
                    Developer::warning('Failed to resolve token for display columns', [
                        'token' => $params['token'],
                        'table' => $table,
                        'error' => $e->getMessage()
                    ]);
            ...
            // Check if columns are directly provided in params
            if (!empty($params['columns'])) {
                ...
                if (is_string($params['columns'])) {
                    ...
                    if (strpos($params['columns'], '|') !== false) {
            ...
            // Default fallback - check if table has common display columns
            $schemaColumns = $this->getCachedSchemaColumns($table);
            $commonDisplayColumns = ['name', 'title', 'label', 'description'];
            
            foreach ($commonDisplayColumns as $col) {
