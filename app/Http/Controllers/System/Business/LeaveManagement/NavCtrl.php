...
class NavCtrl extends Controller
{
    ...
    public function index(Request $request, array $params)
    {
        try {
            ...
            $baseView = 'system.business.leave-management';
            $module = $params['module'] ?? 'leave-management';
            ...
            $blade = end($viewparts);
            Developer::alert('blade is ',['blade'=>$blade]);
            switch ($blade) {
                case 'index':
                    $data['dashboard_list'] = [];
                    break;
                ...
                case 'manage-leave':
                    $params = [
                        'columns' => ['id', 'leave_id', 'status'],
                    ];
                    $response = Data::get('business', 'leave_requests', $params);
                    $data['leave_requests'] = $response['status'] ? $response['data'] : [];
                    break;
            }
            /****************************************************************************************************
