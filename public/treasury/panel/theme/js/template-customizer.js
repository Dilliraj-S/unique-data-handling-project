...
class TemplateCustomizer {
  ...
  _setup(_container = document) {
      ...
      return this._getElementFromString(`<div class="col-4 px-2">
        ...
        <label class="form-check-label custom-option-content p-0" for="${inputName}${nameVal}">
          <span class="custom-option-body mb-0">
            <img src="${assetsPath}/treasury/panel/theme/customizer/${image}${isDarkStyle ? '-dark' : ''
    ...
    this._cleanup()
    this.container = this._getElementFromString(`<div id="template-customizer" class="bg-card">
  ...
  `)
    ...
    if (this.settings.displayCustomizer) customizerW.style.visibility = 'visible'
    else customizerW.style.visibility = 'hidden'
    // Open btn
    //
  ...
  _initStyle() {
    ...
    this._insertStylesheet(
      ...
      this.pathResolver(
        ...
        this.settings.cssFilenamePattern.replace('%name%', `core`)
      )
    )
