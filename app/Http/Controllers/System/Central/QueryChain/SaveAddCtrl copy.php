        ...
        try {
            ...
            switch ($reqSet['key']) {
                ...
                case 'unique_process_logs':
                    ...
                    $skippedTargets = [];
                    if ($toggleLeads && !$hasLeadHeaders) {
                        $skippedTargets[] = 'master_leads (required headers missing)';
                        $toggleLeads = false;
                    }
                    if ($toggleAccounts && !$hasAccountHeaders) {
                        $skippedTargets[] = 'master_accounts (required headers missing)';
                        ...
                        if (!empty($headerSet)) {
                            $query->where(function ($q) use ($headerSet) {
                                foreach ($headerSet as $h) {
                                    $q->orWhere(function ($qq) use ($h) {
                                        $qq->whereNotNull($h)->where($h, '!=', '');
                                    });
                                }
                            });
                        }
