...
export async function select() {
  ...
  /**
   * Checks if a dataset is large by making a count request.
   * @param {jQuery} $el - The jQuery select element.
   * @param {boolean} isDynamic - Whether the select is dynamic.
   * @returns {Promise<boolean>} True if dataset is large (>500 records).
   */
  async function isLargeDataset($el, isDynamic = false) {
    // Check explicit data attributes first
    if ($el.data('large-dataset') === true || $el.data('search-only') === true) {
      return true;
    }

    // Check known large tables
    const largeTables = ['users', 'products', 'orders', 'transactions', 'logs', 'audit_trails'];
    if (largeTables.includes($el.data('table'))) {
      return true;
    }

    // For dynamic selects, make a quick count request
    if (isDynamic && $el.data('source')) {
      try {
        const source = $el.data('source');
        let requestData = {
          table: $el.data('table') || source || null,
          columns: $el.data('columns') || 'id|name',
          initial_load:true,
          count_only: true, // Special flag for count-only requests
          per_page: 1 // Minimal data
        };
        if($el.data('columns')){
          requestData.initial_load=false;
        }

        // Only add skeleton_token if it's a valid 27-character token
        if (source && source.length === 27) {
          requestData.skeleton_token = source;
        }
        ...
        const response = await window.general.axiosRequest({
          ...
          url: '/select/options',
          data: requestData,
          requestId: `select-count-${$el.attr('id') || $el.data('source')}`
        });

        // If response indicates large dataset (>500 records), use search-only mode
        return response?.data?.total_count > 500 || response?.data?.is_large_dataset === true;
      } catch (error) {
        // If count request fails, default to show-all for small datasets
        console.warn('Could not determine dataset size, defaulting to show-all mode:', error);
        return false; // Changed from true to false for better UX
      }
    }

    return false; // Default to show all for small datasets
  }
   ...
   */
  async function initSelect2($el, optionsList, isDynamic = false) {
    ...
    const config = {
      ...
      // Smart minimum input length based on data size
      minimumInputLength: isDynamic ? (await isLargeDataset($el, isDynamic) ? 1 : 0) : 0,
    ...
    if (isDynamic) {
      config.ajax = {
        ...
        data: function (params) {
          ...
          // Only include skeleton_token if it's a valid 27-character token
          const source = $el.data('source');
          let requestData = {
            ...
            selected: parseDataValue($el.data('value')),
            table: $el.data('table') || source || null,
            columns: $el.data('columns') || 'id|name'
          };
   ...
   * @param {Array} optionsList - Array of options for static dropdowns.
   */
  async function initSelect($select, optionsList) {
    try {
      ...
      if (type === 'dropdown') {
        ...
        await initSelect2($select, optionsList);
        // Apply pre-selection
        ...
        await initSelect2($select, [], true); // Pass true for dynamic
      } else {
  ...
  const initPromises = Array.from(selects).map(async (select) => {
    try {
      const $select = $(select);
    ...
    if ($select.data('select') === 'dropdown' || $select.data('select') === 'dynamic') {
      ...
      if (preselectVal.length && !preselectVal.every(val => $select.val()?.includes(val))) {
        if ($select.data('select') === 'dropdown') {
          ...
          initSelect2($select, [], true).catch(e => {
            window.general.error('Error in dynamic pre-selection', { id: $select.attr('id'), error: e.message });
          });
        }
      }
    }
