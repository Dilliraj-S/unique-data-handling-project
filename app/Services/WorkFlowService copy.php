...
public function DlsDesignationsMethod(string $workflowName, array $input, array $output, array $tracing, array $metadata): array
{
    ...
    public function GenerateEmailsMethod(string $workflowName, array $input, array $output, array $tracing, array $metadata): array
    {
        ...
        try {
            ...
            Developer::info("Loaded email format data", [
                'domainFormatMap' => $domainFormatMap,
                'emailFormatsCount' => count($emailFormats),
            ...
            ProcessFlowHelper::readInput($input, function ($row, $index) use (&$rows, &$metrics, $domainFormatMap, $emailFormats, $separatorMap) {
                $metrics['total']++;
                ...
                // Debug logging
                Developer::info("Email format processing", [
                    'domain' => $domain,
                    'formatsToUse' => $formatsToUse,
                    'emailFormatsCount' => count($emailFormats)
                ]);
                ...
                if (empty($formatsToUse)) {
                    ...
                    Developer::info("No domain-specific formats found, using all formats", [
                        'domain' => $domain,
                        'fallbackFormats' => $formatsToUse
                    ]);
                }
                ...
                if (empty($formatsToUse) && !empty($emailFormats)) {
                    ...
                    Developer::info("Using fallback format ID", [
                        'domain' => $domain,
                        'fallbackFormat' => $formatsToUse
                    ]);
                }
                ...
                // Debug: Log if no formats were processed
                if (empty($formatsToUse)) {
                    Developer::warning("No email formats processed for record", [
                        'fullName' => $fullName,
                        'domain' => $domain,
                        'formatsToUse' => $formatsToUse
                    ]);

                    // Add a fallback row to ensure at least one output per input
                    $row['li_full_name'] = $fullName;
                    $row['domain'] = $domain;
                    $row['li_first_name'] = $firstName;
                    $row['li_last_name'] = $lastName;
            ...
            // Debug: Log final row count
            Developer::info("GenerateEmailsMethod completed processing", [
    ...
    //         $baseTable = "{$baseInputDb}.{$baseInputTable}";
    ...
    //         // Step 1: Analyze the base table to find the most frequent dls_email_number
    //         $emailNumberCounts = DB::connection($baseInputDb)->table($baseInputTable)
    //             ->select('dls_email_number', DB::raw('COUNT(*) as count'))
    //             ->whereNotNull('dls_email_number')
    //             ->where('dls_email_number', '!=', '')
    //             ->groupBy('dls_email_number')
    //             ->orderBy('count', 'desc')
    //             ->get();

    //         if ($emailNumberCounts->isEmpty()) {
    //             throw new Exception("No dls_email_number data found in base table {$baseInputDb}.{$baseInputTable}.");
    //         }

    //         $mostFrequentNumber = $emailNumberCounts->first()->dls_email_number;
    //         $maxCount = $emailNumberCounts->first()->count;

    //         Developer::info("Most frequent dls_email_number found", [
    //             'number' => $mostFrequentNumber,
    //             'count' => $maxCount,
    //             'all_counts' => $emailNumberCounts->toArray()
    //         ]);

    //         // Step 2: Get email format configuration for the most frequent number
    //         $emailFormat = DB::table('moon.email_formats')
    //             ->where('status', $mostFrequentNumber)
    //             ->first();

    //         if (!$emailFormat) {
    //             throw new Exception("No email format found for status {$mostFrequentNumber} in moon.email_formats table.");
    //         }

    //         Developer::info("Email format configuration found", [
    //             'status' => $emailFormat->status,
    //             'field_1' => $emailFormat->field_1,
    //             'field_2' => $emailFormat->field_2,
    //             'separator' => $emailFormat->separator
    //         ]);

    //         // Step 3: Define headers for output
    //         $requiredHeaders = ['li_full_name', 'domain'];
    //         $updateHeaders = [
    //             'li_first_name',
    //             'li_last_name',
    ...
    //         // Step 4: Process input data using the discovered email format
    //         $separatorMap = ['Dot' => '.', 'Hyphen' => '-', 'Underscore' => '_', 'Space' => '', 'Empty' => ''];
    //         $separator = $separatorMap[$emailFormat->separator] ?? '';
    ...
    //             // Generate email using the discovered format
    //             $f1 = match ($emailFormat->field_1) {
    //                 'FirstName' => $firstName,
    //                 'LastName' => $lastName,
    //                 'FirstInitial' => $firstInitial,
    //                 'LastInitial' => $lastInitial,
    ...
    //                 $row['li_lastname_initial'] = $lastInitial;
    //                 $row['generated_email'] = '';
    //                 $row['format_code'] = $mostFrequentNumber;
    //                 $row['status'] = 'Bad';
    //                 $row['reason'] = 'Empty name fields';
    //                 $row['__state'] = 'rejected';
    //                 $row['__reason'] = 'Empty name fields';
    ...
    //             $email = strtolower(trim($f1 . $separator . $f2 . '@' . $domain));
    //             $isValid = filter_var($email, FILTER_VALIDATE_EMAIL);
    ...
    //             $row['format_code'] = $mostFrequentNumber;
    //             $row['status'] = $isValid ? 'Good' : 'Bad';
    //             $row['reason'] = $isValid ? 'Valid email generated using auto-discovered format' : 'Invalid email format';
    //             $row['__state'] = $isValid ? 'processed' : 'rejected';
    //             $row['__reason'] = $isValid ? 'completed' : 'Invalid email format';
    //             $outputRows[] = $row;
    ...
    public function AutoflowsMethod(string $workflowName, array $input, array $output, array $tracing, array $metadata): array
    {
        ...
        try {
            ...
            // Prefetch all email formats as a map: status(int) => row
            $emailFormatsAll = DB::table('moon.email_formats')->get()->map(fn($r) => (array) $r)->all();
            $emailFormatsByStatus = [];
            foreach ($emailFormatsAll as $fmt) {
                $key = (int)($fmt['status'] ?? 0);
                if ($key > 0) $emailFormatsByStatus[$key] = $fmt;
            }

            // Caches to avoid repeated queries per domain
            $domainToModeCache = [];            // domain(normalized) => int modeNumber
            $domainFormatsCache = [];           // domain(normalized) => array<int>
            ...
            // Step 4: Process input data; determine format per domain using base table and domain_formats
            $separatorMap = ['Dot' => '.', 'Hyphen' => '-', 'Underscore' => '_', 'Space' => '', 'Empty' => ''];
            ...
            ProcessFlowHelper::readInput($input, function ($row, $index) use (
                ...
                // Determine most repeated dls_email_number (mode) for this domain from base table
                $modeNumber = null;
                if (array_key_exists($domainNorm, $domainToModeCache)) {
                    $modeNumber = $domainToModeCache[$domainNorm];
                } else {
                    $top = DB::connection($baseInputDb)
                        ->table($baseInputTable)
                        ->select('dls_email_number', DB::raw('COUNT(*) as cnt'))
                        ->whereNotNull('li_smtp')
                        ->whereRaw('LOWER(REPLACE(li_smtp, "www.", "")) = ?', [$domainNorm])
                        ->groupBy('dls_email_number')
                        ->orderBy('cnt', 'desc')
                        ->orderBy(DB::raw('CAST(dls_email_number AS UNSIGNED)'), 'desc')
                        ->limit(1)
                        ->first();

                    if ($top && isset($top->dls_email_number)) {
                        $modeNumber = (int) $top->dls_email_number;
                        $domainToModeCache[$domainNorm] = $modeNumber;
                    }
                }
                ...
                if ($modeNumber === null || $modeNumber <= 0) {
                    ...
                    $row['format_code'] = '';
                    $row['status'] = 'skipped';
                    $row['reason'] = 'No base match for domain';
                    $row['__state'] = 'skipped';
                    $row['__reason'] = 'No base match for domain';
                    $outputRows[] = $row;
                    $metrics['skipped']++;
                    return;
                }

                // Check moon.domain_formats for allowed formats per domain
                $allowedFormats = [];
                if (array_key_exists($domainNorm, $domainFormatsCache)) {
                    $allowedFormats = $domainFormatsCache[$domainNorm];
                } else {
                    $df = DB::table('moon.domain_formats')
                        ->whereNull('deleted_at')
                        ->where('domain', $domainNorm)
                        ->first();
                    if ($df && isset($df->email_format) && $df->email_format !== '') {
                        $allowedFormats = array_values(array_filter(array_map(function ($v) {
                            $n = (int) trim($v);
                            return $n > 0 ? $n : null;
                        }, explode(',', (string) $df->email_format))));
                    }
                    $domainFormatsCache[$domainNorm] = $allowedFormats;
                }

                // Choose the format number: prefer modeNumber if present in allowedFormats; else first allowed; else modeNumber
                ...
                if (!$formatDef) {
                    ...
                    $row['domain'] = $domainNorm;
                    $row['generated_email'] = '';
                    $row['format_code'] = (string) $chosenFormatNum;
                    $row['status'] = 'rejected';
                    $row['reason'] = "Email format not defined for status {$chosenFormatNum}";
                    $row['__state'] = 'rejected';
                    $row['__reason'] = $row['reason'];
                    $outputRows[] = $row;
                    $metrics['rejected']++;
                    return;
                }
