...
class GoogleAuthController extends Controller
{
    ...
    public function redirect(Request $request)
    {
        ...
        Log::info("Redirecting to Google from /panels/admin/more/mail-config, session: " . json_encode(Session::all()));
    ...
    public function callback(Request $request)
    {
        ...
        try {
            ...
            // Store the full token (including refresh_token) in storage
            ...
            // Log all pending records before update
            $allPending = $db->select('SELECT * FROM email_accounts WHERE status = ?', ['pending']);
            Log::info('DEBUG: All pending records before update: ' . json_encode($allPending));
            // Find and update the last pending record
            $lastPending = $db->select('SELECT id FROM email_accounts WHERE status = ? ORDER BY id DESC LIMIT 1', ['pending'])[0] ?? null;
            Log::info('DEBUG: GoogleAuthController callback $lastPending = ' . json_encode($lastPending));

            if ($lastPending) {
                $db->update(
                    'UPDATE email_accounts SET email = ?, type = ?, access_token = ?, refresh_token = ?, status = ?, updated_at = NOW() WHERE id = ?',
                    [$email, 'google', $token['access_token'], $token['refresh_token'] ?? null, 'active', $lastPending->id]
                );
            } else {
                // If no pending record, insert new record
                $db->insert(
                    'INSERT INTO email_accounts (user_id, type, email, access_token, refresh_token, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())',
            ...
            return redirect('/engage')->with('success', "Authenticated $email successfully.");
        } catch (\Exception $e) {
            Log::error("Google OAuth callback failed: " . $e->getMessage());
            return redirect('/panel/admin/more/mail-config')
