...
export async function select() {
   ...
   */
  async function isLargeDataset($el) {
    ...
    if (isDynamic && $el.data('source')) {
      try {
        ...
        // If count request fails, default to search-only for safety
        console.warn('Could not determine dataset size, defaulting to search-only mode:', error);
        return true;
      }
    }
  ...
  async function initSelect2($el, optionsList, isDynamic = false) {
    ...
    const config = {
      ...
      minimumInputLength: isDynamic ? (await isLargeDataset($el) ? 1 : 0) : 0,
