...
class MasterFlowService
{
    ...
    public function processFlow(array $flows, array $flowData): array
    {
        ...
        // Optimized database settings for massive file processing (100M-200M rows)
        try {
            $dbConnection->statement("SET NAMES 'utf8mb4'");
            $dbConnection->statement('SET SESSION sql_big_selects = 1');
            
            // Extended timeouts for massive files (48 hours)
            $dbConnection->statement('SET SESSION wait_timeout = 172800');
            $dbConnection->statement('SET SESSION interactive_timeout = 172800');
            $dbConnection->statement('SET SESSION net_read_timeout = 3600');
            $dbConnection->statement('SET SESSION net_write_timeout = 3600');
            
            // Performance optimizations for bulk operations
            $dbConnection->statement('SET SESSION max_allowed_packet = 1073741824'); // 1GB
            $dbConnection->statement('SET SESSION bulk_insert_buffer_size = 268435456'); // 256MB
            $dbConnection->statement('SET SESSION innodb_buffer_pool_size = 8589934592'); // 8GB (if allowed)
            
            // Disable checks for faster bulk inserts (RISKY - only for trusted data)
            if (config('large_file_processing.database_optimization.disable_foreign_keys', false)) {
                $dbConnection->statement('SET SESSION foreign_key_checks = 0');
                $dbConnection->statement('SET SESSION unique_checks = 0');
                Log::info('⚠️ Foreign key and unique checks DISABLED for faster bulk operations');
            }
            
            // Transaction isolation for better concurrency
            $dbConnection->statement('SET SESSION transaction_isolation = "READ-COMMITTED"');
            
            Log::info('✅ Database optimizations applied for massive file processing');
        } catch (\Throwable $e) {
            Log::warning('⚠️ Some database optimizations could not be applied: ' . $e->getMessage());
            // Continue anyway - driver may not support these pragmas
        }
        ...
        try {
            ...
            try {
                ...
                // Optimized partitioning for massive files (100M-200M rows)
                // 10M rows per partition = 20 partitions for 200M rows
                $createTableSql .= "
                PARTITION BY RANGE (id) (
                    PARTITION p00 VALUES LESS THAN (10000000),
                    PARTITION p01 VALUES LESS THAN (20000000),
                    PARTITION p02 VALUES LESS THAN (30000000),
                    PARTITION p03 VALUES LESS THAN (40000000),
                    PARTITION p04 VALUES LESS THAN (50000000),
                    PARTITION p05 VALUES LESS THAN (60000000),
                    PARTITION p06 VALUES LESS THAN (70000000),
                    PARTITION p07 VALUES LESS THAN (80000000),
                    PARTITION p08 VALUES LESS THAN (90000000),
                    PARTITION p09 VALUES LESS THAN (100000000),
                ...
                Log::info("✅ Created partitioned cloned table {$fullClonedTable}");
            } catch (Exception $e) {
            ...
            // Prepare cloned table column metadata once
            ...
            // Insert input data in batches using streaming (avoid building all rows in memory)
            // MySQL prepared statement placeholders hard cap ~65,535. Respect this to avoid 1390 errors.
            $placeholderCap = 60000;
            $columnCount = count($tableColumns);
            $batchSize = max(500, (int) floor($placeholderCap / max(1, $columnCount)));
            $insertBatch = [];

            if ($processMode === 'flow' && ($input['type'] ?? 'csv') === 'csv' && empty($input['rows'])) {
                $rowIndexLocal = 0;
                ProcessFlowHelper::readInput($input, function ($row) use (&$insertBatch, &$pipelineMetrics, &$rowIndexLocal, $dbConnection, $tableColumns, $datetimeColumnNames, $clonedTable, $processId, $batchSize, $columnCount, $placeholderCap) {
                    $insertRow = array_fill_keys($tableColumns, null);

                    // Let DB assign auto-increment id if missing; keep if present
                    ...
                    foreach ($row as $key => $value) {
                        if (in_array($key, $tableColumns, true)) {
                            if (is_string($value) && !mb_check_encoding($value, 'UTF-8')) {
                                ...
                                if ($rowIndexLocal < 3) {
                                    Log::warning('Invalid UTF-8 string detected, converted to UTF-8', [
                                        'process_id' => $processId,
                                        'column' => $key
                                    ]);
                                }
                            }
                            if ($value === '' || (in_array($key, $datetimeColumnNames, true) && !strtotime((string) $value))) {
                                $insertRow[$key] = null;
                            } else {
                                $insertRow[$key] = $value;
                            }
                ...
                if (count($insertBatch) >= $batchSize) {
                    foreach (array_chunk($insertBatch, $batchSize) as $chunk) {
                        // Safety: split further if placeholders exceed cap
                        $rowsInChunk = count($chunk);
                        $placeholders = $rowsInChunk * $columnCount;
                        if ($placeholders > $placeholderCap) {
                            $safeSize = max(1, (int) floor($placeholderCap / max(1, $columnCount)));
                            foreach (array_chunk($chunk, $safeSize) as $sub) {
                                $dbConnection->table("moon.$clonedTable")->insert($sub);
                                $pipelineMetrics['total'] += count($sub);
                            }
                        } else {
                ...
                // Non-CSV or rows already provided: batch insert without keeping all in memory again
                if (empty($input['rows'])) {
            ...
            // Add helpful indexes after bulk insert to accelerate following updates
            try {
                $dbConnection->statement("ALTER TABLE `moon`.`{$clonedTable}` ADD INDEX idx_status (status), ADD INDEX idx_id (id)");
            } catch (\Throwable $e) {
                Log::warning('Index creation failed or already exists', ['table' => $fullClonedTable, 'error' => $e->getMessage()]);
            }
            ...
            foreach ($flows as $workflowName => $method) {
                ...
                $batchSize = (int) config('large_file_processing.batch_size', 5000);
                $lastId = 0;
                $totalProcessed = 0;
                // Incremental metrics per workflow to avoid storing all rows in memory
                $workflowMetrics = ['total' => 0, 'processed' => 0, 'rejected' => 0, 'skipped' => 0];
                ...
                do {
                    ...
                    try {
                        ...
                        // Update incremental metrics for this batch
                        foreach ($workflowRows as $wr) {
                            $workflowMetrics['total']++;
                            $state = strtolower($wr['workflow_status'] ?? 'processed');
                            if (isset($workflowMetrics[$state])) {
                                $workflowMetrics[$state]++;
                            }
                        }
                        ...
                        foreach ($workflowRows as $row) {
                            ...
                            // Use pre-normalized headers from metadata when available to avoid repeated decoding
                            $methodLevelConfig = collect($metadata['workflow_map'] ?? [])->first(function ($cfg) use ($workflowName) {
                                return is_array($cfg) && strcasecmp(trim($cfg['workflow_name'] ?? ''), $workflowName) === 0;
                            }) ?? [];
                            $updateHeaders = $methodLevelConfig['update_headers'] ?? (json_decode($workflowConfig->update_headers ?? '[]', true) ?: []);
                        ...
                        foreach (array_chunk($updateBatch, 2000) as $chunk) {
                            $caseStatements = [];
                            $ids = [];
                ...
                // Merge incremental workflow metrics into pipeline metrics
    ...
    public function DlsDesignationsMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        try {
            ...
            // ✅ Create temporary table (with composite UNIQUE to deduplicate keys)
            ...
            $unique = 'UNIQUE KEY uniq_map (' . implode(',', array_map(fn($c) => "`$c`", $mappingHeaders)) . ')';
            DB::connection('central')->statement("CREATE TEMPORARY TABLE $tempTable ($cols, $indexes, $unique) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");

            // ✅ Insert unique normalized keys into temp table with dynamic chunking
            ...
            foreach ($input['rows'] as $row) {
                ...
                if (count($insertBatch) >= $batchSize) {
                    ...
                    DB::connection('central')->statement(
                        "INSERT IGNORE INTO $tempTable (" . implode(',', $mappingHeaders) . ") VALUES $placeholders",
            ...
            // ✅ Perform a single join with support table to build mapping
    ...
    public function GmseMappingMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        Log::info("📥 Starting GmseMappingMethod (ULTRA-OPTIMIZED)", [
            'workflow' => $workflowName,
            'input_row_count' => count($input['rows'])
        ]);
        ...
        try {
            ...
            // ✅ OPTIMIZATION 1: Create temp table with MEMORY engine for faster operations
            ...
            // Use MEMORY engine for small datasets (faster), InnoDB for large
            $tableEngine = count($input['rows']) < 1000 ? 'MEMORY' : 'InnoDB';
            DB::connection($gmseConn)->statement("CREATE TEMPORARY TABLE $tempTable ($cols, $indexes, $unique) ENGINE=$tableEngine DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");

            // ✅ OPTIMIZATION 2: Bulk insert in single statement (faster)
            $maxPlaceholders = 10000;
            $columnCount = count($mappingHeaders);
            $batchSize = max(1, floor($maxPlaceholders / $columnCount));
            $insertBatch = [];
            
            // Pre-filter and deduplicate keys in PHP (faster than INSERT IGNORE)
            $uniqueKeys = [];
            foreach ($input['rows'] as $row) {
            ...
            // Bulk insert unique keys only
            if (!empty($insertBatch)) {
                foreach (array_chunk($insertBatch, $batchSize) as $chunk) {
                    $placeholders = implode(',', array_fill(0, count($chunk), '(' . implode(',', array_fill(0, $columnCount, '?')) . ')'));
            ...
            // ✅ OPTIMIZATION 3: Optimized JOIN with STRAIGHT_JOIN and index hints
            $on = implode(' AND ', array_map(fn($col) => "t.`$col` = s.`$col`", $mappingHeaders));
            $select = implode(', ', array_merge(
                array_map(fn($col) => "s.`$col`", $updateHeaders),
                array_map(fn($col) => "t.`$col`", $mappingHeaders)
            ...
            // Use STRAIGHT_JOIN to force join order (temp table first is faster)
            // Add USE INDEX hints if you know the index names on support table
            $optimizedQuery = "
                SELECT STRAIGHT_JOIN $select 
                FROM $tempTable t 
                LEFT JOIN $qualifiedGmseTable s 
                ON $on
            ";
            
            $results = DB::connection($gmseConn)->select($optimizedQuery);

            // ✅ OPTIMIZATION 4: Build hashmap with pre-allocated size
            $mappedData = [];
            foreach ($results as $res) {
                $rowArray = (array)$res;
                $lookupKey = implode('|', array_map(fn($col) => strtolower(trim($rowArray[$col] ?? '')), $mappingHeaders));
            ...
            Log::debug('🔍 GMSE Lookup Results', [
                'unique_keys' => count($uniqueKeys),
                'matched_records' => count($mappedData),
                'match_rate' => count($mappedData) > 0 ? round(count($mappedData) / count($uniqueKeys) * 100, 2) . '%' : '0%'
            ]);

            // ✅ OPTIMIZATION 5: Process rows with minimal array operations
            foreach ($input['rows'] as $row) {
                $metrics['total']++;
                $outputRow = $row;
                ...
                // Build lookup key (cached from earlier if possible)
                $keyParts = array_map(function ($i) use ($row, $requiredHeaders) {
                    return strtolower(trim($row[$requiredHeaders[$i]] ?? ''));
                }, array_keys($mappingHeaders));
                $lookupKey = implode('|', $keyParts);
            ...
            Log::info("✅ Completed GmseMappingMethod (ULTRA-OPTIMIZED)", [
                'workflow' => $workflowName,
                'metrics' => $metrics,
                'duration_seconds' => $duration,
                'rows_per_second' => $metrics['total'] > 0 ? round($metrics['total'] / $duration, 2) : 0
            ]);
        } catch (\Throwable $e) {
    ...
    public function ApolloMappingMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        Log::info("📥 Starting ApolloMappingMethod (ULTRA-OPTIMIZED)", [
            'workflow' => $workflowName,
            'input_row_count' => count($input['rows'])
        ]);
        ...
        try {
            ...
            Log::debug('🔎 ApolloMappingMethod config', [
                'workflow' => $workflowName,
                'support_table' => $supportTable,
                'mapping_columns' => count($mappingHeaders) . ' (4-column join - may be slow)'
            ]);
            ...
            // Use MEMORY engine for small datasets (faster), InnoDB for large
            $tableEngine = count($input['rows']) < 1000 ? 'MEMORY' : 'InnoDB';
            DB::connection('central')->statement("CREATE TEMPORARY TABLE $tempTable ($cols, $indexes, $unique) ENGINE=$tableEngine DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");

            // ✅ OPTIMIZATION 2: Bulk insert with PHP deduplication
            $maxPlaceholders = 10000;
            $columnCount = count($mappingHeaders);
            $batchSize = max(1, floor($maxPlaceholders / $columnCount));
            $insertBatch = [];
            ...
            // ✅ OPTIMIZATION 3: Optimized JOIN with STRAIGHT_JOIN
            $on = implode(' AND ', array_map(fn($col) => "t.`$col` = s.`$col`", $mappingHeaders));
            $select = implode(', ', array_merge(
                array_map(fn($col) => "s.`$col`", $updateHeaders),
                array_map(fn($col) => "t.`$col`", $mappingHeaders)
            ...
            $optimizedQuery = "
                SELECT STRAIGHT_JOIN $select 
                FROM $tempTable t 
                LEFT JOIN $supportTable s 
                ON $on
            ";
            
            $results = DB::connection('central')->select($optimizedQuery);

            // ✅ OPTIMIZATION 4: Build hashmap efficiently
            $mappedData = [];
            foreach ($results as $res) {
                $rowArray = (array)$res;
                $lookupKey = implode('|', array_map(fn($col) => strtolower(trim($rowArray[$col] ?? '')), $mappingHeaders));
            ...
            Log::debug('🔍 Apollo Lookup Results', [
                'unique_keys' => count($uniqueKeys),
                'matched_records' => count($mappedData),
                'match_rate' => count($mappedData) > 0 ? round(count($mappedData) / count($uniqueKeys) * 100, 2) . '%' : '0%'
            ]);

            // ✅ OPTIMIZATION 5: Process rows efficiently
            foreach ($input['rows'] as $row) {
                $metrics['total']++;
                $outputRow = $row;
            ...
            Log::info("✅ Completed ApolloMappingMethod (ULTRA-OPTIMIZED)", [
                'workflow' => $workflowName,
                'metrics' => $metrics,
                'duration_seconds' => $duration,
                'rows_per_second' => $metrics['total'] > 0 ? round($metrics['total'] / $duration, 2) : 0
    ...
    public function ZoomInfoMappingMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        Log::info("📥 Starting ZoomInfoMappingMethod (ULTRA-OPTIMIZED)", [
            'workflow' => $workflowName,
            'input_row_count' => count($input['rows'])
        ]);
        ...
        try {
            ...
            DB::connection($zmConn)->statement(
                "CREATE TEMPORARY TABLE $tempTable ($cols, $indexes, $unique)
             ENGINE=$tableEngine DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
            );
            ...
            if (!empty($insertBatch)) {
                foreach (array_chunk($insertBatch, $batchSize) as $chunk) {
                    $placeholders = implode(',', array_fill(0, count($chunk), '(' . implode(',', array_fill(0, $colCount, '?')) . ')'));
                    DB::connection($zmConn)->statement(
                        "INSERT INTO $tempTable (" . implode(',', $mappingHeaders) . ") VALUES $placeholders",
                        array_merge(...$chunk)
                    );
            ...
            $optimizedQuery = "
                SELECT STRAIGHT_JOIN $select 
                FROM $tempTable t 
                LEFT JOIN $qualifiedSupportTable s 
                ON $on
            ";
            
            $results = DB::connection($zmConn)->select($optimizedQuery);

            // ✅ OPTIMIZATION 4: Build in-memory map efficiently
            $mappedData = [];
            foreach ($results as $res) {
                $rowArray = (array)$res;
                $lookupKey = implode('|', array_map(fn($col) => strtolower(trim($rowArray[$col] ?? '')), $mappingHeaders));
            ...
            Log::debug('🔍 ZoomInfo Lookup Results', [
                'unique_keys' => count($uniqueKeys),
                'matched_records' => count($mappedData),
                'match_rate' => count($mappedData) > 0 ? round(count($mappedData) / count($uniqueKeys) * 100, 2) . '%' : '0%'
            ]);

            // ✅ OPTIMIZATION 5: Process each row efficiently
            foreach ($input['rows'] as $row) {
                $metrics['total']++;
                $outputRow = $row;
            ...
            Log::info("✅ Completed ZoomInfoMappingMethod (ULTRA-OPTIMIZED)", [
                'workflow' => $workflowName,
                'metrics'  => $metrics,
                'duration_seconds' => $duration,
                'rows_per_second' => $metrics['total'] > 0 ? round($metrics['total'] / $duration, 2) : 0
        ...
        try {
            ...
            foreach ($input['rows'] as $row) {
                ...
                if (count($insertBatch) >= $batchSize) {
                    ...
                    DB::connection('central')->statement("INSERT IGNORE INTO $tempTable (" . implode(',', $mappingHeaders) . ") VALUES $ph", array_merge(...$insertBatch));
                    $insertBatch = [];
                }
            }
            if ($insertBatch) {
