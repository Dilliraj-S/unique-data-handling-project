...
public function DlsDesignationsMethod(string $workflowName, array $input, array $output, array $tracing, array $metadata): array
{
    ...
    public function AutoFlowMethod(string $workflowName, array $input, array $output, array $tracing, array $metadata): array
    {
        Developer::info("📥 Received to AutoFlowMethod", [
            'workflow' => $workflowName,
            'input' => $input,
            'output' => $output,
            'metadata' => $metadata,
        ...
        try {
            // Get workflow configuration from metadata
            $workflowConfig = collect($metadata['workflow_map'] ?? [])
                ->first(fn($config) => strcasecmp(trim($config['workflow_name'] ?? ''), trim($workflowName)) === 0);
            ...
            $supportTable = $workflowConfig['support_table'] ?? 'moon.email_formats';
            $requiredHeaders = $workflowConfig['required_headers'] ?? ['li_full_name', 'li_smtp'];
            $mappingHeaders = $workflowConfig['mapping_headers'] ?? ['dls_email_number'];
            $updateHeaders = $workflowConfig['update_headers'] ?? [
                'li_first_name',
                'li_last_name', 
                'li_firstname_initial',
            ...
            // Step 1: Find the most frequent email format from dls_email_number column
            Developer::info("🔍 Finding most frequent email format from dls_email_number column");
            
            $formatFrequency = [];
            $allRows = [];
            
            // First pass: collect all dls_email_number values and count frequency
            ProcessFlowHelper::readInput($input, function ($row, $index) use (&$formatFrequency, &$allRows) {
                $emailFormat = trim($row['dls_email_number'] ?? '');
                if (!empty($emailFormat)) {
                    $formatFrequency[$emailFormat] = ($formatFrequency[$emailFormat] ?? 0) + 1;
                }
                $allRows[] = $row; // Store all rows for second pass
            });

            if (empty($formatFrequency)) {
                throw new Exception("No email formats found in dls_email_number column");
            }

            // Find the most frequent format (mode)
            arsort($formatFrequency); // sort desc by count, preserve keys
            $mostFrequentFormat = array_key_first($formatFrequency);
            $maxCount = reset($formatFrequency);
            
            Developer::info("📊 Most frequent email format found", [
                'format' => $mostFrequentFormat,
                'count' => $maxCount,
                'total_formats' => count($formatFrequency)
            ]);

            // Step 2: Load email format configuration from support table
            $parts = explode('.', $supportTable);
            $supportDb = $parts[0];
            $supportTableName = $parts[1] ?? $supportTable;

            // Load email format configuration for the most frequent format
            $formatConfig = DB::connection('central')
                ->table("$supportDb.$supportTableName")
                ->where('status', $mostFrequentFormat)
                ->first();

            if (!$formatConfig) {
                throw new Exception("Email format configuration not found for format: $mostFrequentFormat");
            }

            $formatConfig = (array) $formatConfig;
            Developer::info("📋 Loaded email format configuration", [
                'format_id' => $mostFrequentFormat,
                'field_1' => $formatConfig['field_1'] ?? '',
                'field_2' => $formatConfig['field_2'] ?? '',
            ...
            // Step 3: Process all rows using the most frequent format
            foreach ($allRows as $row) {
                ...
                // Extract domain from li_smtp (and also expose as 'domain' field for output)
                $smtp = trim($row['li_smtp'] ?? '');
                $domain = '';
                if (!empty($smtp) && strpos($smtp, '@') !== false) {
                    $domain = strtolower(trim(explode('@', $smtp)[1] ?? ''));
                }
                $row['domain'] = $domain;

                if (empty($domain)) {
                    $row['status'] = 'rejected';
                    $row['reason'] = 'No valid domain found in li_smtp';
                    $metrics['rejected']++;
                    $outputRows[] = $row;
                    continue;
                }
                ...
                // Generate email using the most frequent format
                $sep = $separatorMap[$formatConfig['separator']] ?? '';

                $f1 = match ($formatConfig['field_1']) {
                    'FirstName' => $firstName,
                    'LastName' => $lastName,
                    'FirstInitial' => $firstInitial,
                    'LastInitial' => $lastInitial,
                ...
                if ($f1 === '' && $f2 === '') {
                    $row['status'] = 'rejected';
                    $row['reason'] = 'Empty name fields in format configuration';
                    $metrics['rejected']++;
                    $outputRows[] = $row;
                    continue;
                }
                ...
                $row['li_lastname_initial'] = $lastInitial;
                $row['generated_email'] = $email;
                $row['format_code'] = $mostFrequentFormat;
                $row['status'] = $isValid ? 'Good' : 'Bad';
                $row['reason'] = $isValid ? 'Valid email generated using most frequent format' : 'Invalid email format';
            ...
            Developer::error("❗ Error in AutoFlowMethod", ['error' => $e->getMessage()]);
            return ProcessFlowHelper::addTraceEntry(
                $workflowName,
                'failed',
                $metrics,
        ...
        $traceEntry = ProcessFlowHelper::addTraceEntry(
            ...
            'completed',
            $metrics,
            "Processed `$workflowName` using most frequent format"
        );

        Developer::info("✅ Completed AutoFlowMethod", [
            'workflow' => $workflowName,
            'trace_entry' => $traceEntry
        ]);
