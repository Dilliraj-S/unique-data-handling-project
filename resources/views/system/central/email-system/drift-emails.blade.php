...
@section('top-style')
    ...
    <style>
        ...
        .nav-pills .nav-link:hover:not(.active) {
            ...
            border-color: rgba(29, 180, 205, 0.2);
...
@section('content')
    <div class="container">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3 shadow-sm px-4 py-3"
                ...
                <div class="d-flex align-items-center gap-2">
                    <button class="btn btn-outline-primary btn-sm sharp-btn" id="show-quota-btn"
                        ...
                        <i id="refresh-quota-btn" class="bi bi-arrow-clockwise ms-2"
                            style="font-size:1.2em; color:#1db4cd; cursor:pointer; vertical-align:middle;"
                            title="Refresh Quota"></i>
                    </button>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <ul class="nav nav-pills data-skl-action" id="setTabs" role="tablist">
                        <!-- Dynamically populated by JavaScript -->
                    </ul>
                    <!-- <div class="action-area">
                        <button class="btn btn-primary" id="drift-action-btn">New Set</button>
                    </div> -->
                </div>
                <div class="tab-content" id="setTabContent">
                    <div class="tab-pane fade show active" id="set-content-active" role="tabpanel"
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <ul class="nav nav-pills data-skl-action" id="sequenceTabs" role="tablist">
                                <!-- Dynamically populated by JavaScript -->
                            </ul>
                            <div class="action-area">
                                <button class="btn btn-primary" id="sequence-action-btn">New Sequence</button>
                            </div>
                        </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content" id="createAudienceTabContent">
                            ...
                            <div class="tab-pane fade" id="import-csv-audience" role="tabpanel"
                                ...
                                <div class="progress mt-3" id="csv-upload-progress" style="display: none;">
        ...
        <!-- Add this after repliedEmailsModal -->
        <div class="modal fade" id="emailBodyModal" tabindex="-1" aria-labelledby="emailBodyModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body" id="email-body-modal-content">
                        <!-- Email body will be injected here -->
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    </div>
        ...
        <!-- 1. Add a new modal for column mapping and preview after the existing modals -->
        <div class="modal fade" id="csvMappingModal" tabindex="-1" aria-labelledby="csvMappingModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="csvMappingModalLabel">Map CSV Columns & Preview</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div id="csv-mapping-section"></div>
                        <div id="csv-preview-section" class="mt-4"></div>
                        <div id="csv-mapping-error" class="text-danger mt-2"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    </div>
...
@section('bottom-script')
    ...
    <script>
        ...
        let allowTabSwitch = false; // Global variable to control tab switching
        ...
        // Replace polling logic for quota
        ...
        function startQuotaPolling() {
            ...
            quotaPollingInterval = setInterval(fetchAndDisplayQuotass, 10000); // 10 seconds
        }
        $(document).ready(function() {
            startQuotaPolling();
            // Remove the old refresh button handler
            $('#refresh-quota-btn').remove();
            ...
            $('#quotaModal').off('show.bs.modal').on('show.bs.modal', function() {
                ...
                let html = '<table class="table table-bordered table-sm"><thead><tr><th>Email</th><th>Daily Limit</th><th>Sent in Last 24h</th></tr></thead><tbody>';
                accounts.forEach(acc => {
                    html += `<tr><td>${acc.email}</td><td>${acc.daily_send_limit ?? 'N/A'}</td><td>${acc.sent_in_last_24h ?? 0}</td></tr>`;
                });
                html += '</tbody></table>';
                $body.html(html);
            });
        ...
        $(document).ready(function() {
            ...
            function populateSequenceForm(setId, tabIndex, sequence) {
                ...
                if (tabIndex === 1) {
                    ...
                    const assignmentMode = (sequence.assignment_mode === undefined || sequence.assignment_mode ===
                        null || sequence.assignment_mode === '') ? 'batch_size' : sequence.assignment_mode;
...
`);
        ...
        <li class="nav-item">
            <button class="nav-link ${isActive}" id="sequence-tab-${setId}-${tabIndex}" data-bs-toggle="tab"
                ...
                data-skl-action="b" data-token="drift_sequence_${setId}_${tabIndex}" data-target="#sequence-action-btn-${setId}" 
                data-type="edit" data-text="Edit Sequence" data-class="btn-primary">
                Sequence ${tabIndex}
            </button>
        </li>
    `;
    ...
    `;
        ...
        `);
                ...
                // PATCH: Immediately initialize report filters select for new tab (if not first tab)
                if (tabIndex > 1) {
                    setTimeout(() => {
                        fetchReportCategories(setId, tabIndex, sequenceId);
                    }, 0);
                }
            }
            ...
            function saveSequenceAndNavigate(setId, tabIndex, direction) {
                ...
                const $navBtn = direction === 'next' ? $(
                    ...
                    $(`.save-sequence-final[data-set-id='${setId}'][data-tab-index='${tabIndex}']`);
                ...
                axios.post('/drift/save-sequences', {
                    ...
                    .then(response => {
                        ...
                        // Only navigate if direction is next or prev
            ...
            function createSetTab(setId, setName, isActive = false) {
                ...
                const setTab = `
                    <li class="nav-item d-flex align-items-center">
                        <button class="nav-link ${isActiveClass}" id="set-tab-${setId}" data-bs-toggle="tab"
                            ...
                            data-skl-action="b" data-token="drift_set_${setId}" data-target="#drift-action-btn" 
                            data-type="manage" data-text="Manage Set" data-class="btn-primary">
                            ${setName}
                        </button>
                ...
                const setContent = `
                    <div class="tab-pane fade ${isShowActive}" id="set-content-${setId}" role="tabpanel" aria-labelledby="set-tab-${setId}">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <ul class="nav nav-pills data-skl-action" id="sequenceTabs-${setId}" role="tablist">
                                <!-- Dynamically populated by JavaScript -->
                            </ul>
                            <div class="action-area">
                                <button class="btn btn-primary" id="sequence-action-btn-${setId}">New Sequence</button>
                            </div>
                        </div>
                ...
                axios.get(`/drift/sequences?set_id=${setId}`)
                    .then(response => {
                        ...
                        if (isActive) {
                            ...
                            // Reset allowTabSwitch when switching to a new active set
                            allowTabSwitch = false;
                        }
            ...
            $('#save-template-btn').on('click', function() {
                ...
                axios.post('/drift/templates', {
                    ...
                    .then(response => {
                        ...
                        // PATCH: Refresh template select in current tab
                        const $activeTab = $('.tab-pane.drift-sequence-box.active');
                        if ($activeTab.length) {
                            const setId = $activeTab.data('set');
                            const tabIndex = $activeTab.attr('id') ? parseInt($activeTab.attr('id')
                                .split('-')[2]) : null;
                            if (setId && tabIndex) {
                                const $templateSelect = $(`#sequence-template-${setId}-${tabIndex}`);
        ...
        `);
            ...
            $('#save-audience-from-campaign-btn').click(function(e) {
                ...
                const subscribersRaw = $('#subscribers-input-create').val();
                $btn.prop('disabled', true).find('.loading-spinner').show();
                ...
                if (!validateSubscribers(subscribersRaw, format)) {
                    toastr.error(
                ...
                // Parse subscribers into array of objects
                let subscribers = [];
                if (subscribersRaw.trim() !== '') {
                    const lines = subscribersRaw.split('\n').map(line => line.trim()).filter(line =>
                        line !== '');
                    subscribers = lines.map(line => {
                        const parts = line.split(',').map(p => p.trim());
                        if (format === 'first-last-email' && parts.length === 3) {
                            return {
                                first_name: parts[0],
                                last_name: parts[1],
                                email: parts[2]
                            };
                        } else if (format === 'first-email' && parts.length === 2) {
                            return {
                                first_name: parts[0],
                                email: parts[1]
                            };
            ...
            $('#import-csv-from-campaign-btn').off('click').on('click', function(e) {
                ...
                $('#csv-upload-progress .progress-bar').css('width', '0%').attr('aria-valuenow', 0).text(
                    '0%');
                // Backend-driven upload with progress bar
                const formData = new FormData();
                formData.append('file', csvFile);
                $.ajax({
                    url: '/drift/audiences/upload-csv',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                        ...
                        xhr.upload.addEventListener('progress', function(evt) {
                            if (evt.lengthComputable) {
                                var percent = Math.round((evt.loaded / evt.total) *
                                    100);
                                $('#csv-upload-progress').show();
                                $('#csv-upload-progress .progress-bar')
                                    .css('width', percent + '%')
                    ...
                    beforeSend: function() {
                        $('#csv-upload-progress').show();
                        $('#csv-upload-progress .progress-bar').css('width', '0%').attr(
                            'aria-valuenow', 0).text('0%');
                    },
                    success: function(res) {
                        $('#csv-upload-progress').hide();
                        $btn.prop('disabled', false);
                        showCsvMappingModal(res.header, res.sample, res.row_count, res.token);
                    },
                    error: function(xhr) {
                        $('#csv-upload-progress').hide();
                        $btn.prop('disabled', false);
                        toastr.error('Failed to read CSV: ' + (xhr.responseJSON?.error ||
                            'Unknown error'));
                    }
                });
            });
            ...
            // Mapping modal logic
            function showCsvMappingModal(header, sample, rowCount, token) {
                ...
                header.forEach((col, idx) => {
                    mappingHtml +=
                        `<div class="col"><label>Column ${idx+1}: <strong>${col}</strong></label><select class="form-select csv-map-select" data-col="${idx}">`;
                ...
                mappingHtml += `<div class="csv-range-container mb-3">
                    <div class="row align-items-center">
                        <div class="col-md-4">
                            <span class="text-muted"><strong>Total rows:</strong> <span id="csv-total-rows" class="badge bg-primary">${rowCount.toLocaleString()}</span></span>
                        </div>
                        <div class="col-md-6">
                            <label class="mb-0 d-flex align-items-center gap-2">
                                <span>Import rows from</span>
                                <input type="number" min="1" max="${rowCount}" value="1" id="csv-row-range-from" 
                                       style="min-width:80px;width:auto;max-width:150px;" 
                                       class="form-control form-control-sm" 
                                       placeholder="1" />
                                <span>to</span>
                                <input type="number" min="1" max="${rowCount}" value="${rowCount}" id="csv-row-range-to" 
                                       style="min-width:80px;width:auto;max-width:150px;" 
                                       class="form-control form-control-sm" 
                        ...
                        <div class="col-md-2">
                            <button type="button" class="btn btn-outline-primary btn-sm w-100" id="csv-select-all-btn">
                ...
                // Sample preview
                mappingHtml += '<div id="csv-preview-table-container" style="max-height:200px;overflow-y:auto;">';
                ...
                mappingHtml +=
                    ...
                    <button type="button" class="btn btn-primary" id="import-mapped-csv-btn">Import</button>
                    <span id="csv-import-progress" style="display:none;" class="text-muted">
                        <span class="spinner-border spinner-border-sm me-1"></span>
                        <span id="csv-import-progress-text">Starting...</span>
                    </span>
                </div>
                <div id="csv-import-results"></div>`;
                $('#csv-mapping-section').html(mappingHtml);
                $('#csvMappingModal').modal('show');
                // Select All button logic
                $('#csv-select-all-btn').off('click').on('click', function() {
                    $('#csv-row-range-from').val(1);
                    $('#csv-row-range-to').val(rowCount);
                });

                // Auto-resize range inputs based on content
                function resizeRangeInputs() {
                    $('#csv-row-range-from, #csv-row-range-to').each(function() {
                        const input = $(this);
                ...
                $('#csv-row-range-from, #csv-row-range-to').off('input').on('input', resizeRangeInputs);

                // Initial resize
                setTimeout(resizeRangeInputs, 100);

                // Add CSS for better input styling
                if (!$('#csv-range-inputs-style').length) {
                    $('head').append(`
                        <style id="csv-range-inputs-style">
                            #csv-row-range-from, #csv-row-range-to {
                                transition: width 0.2s ease;
                                text-align: center;
                                font-weight: 500;
                            }
                            #csv-row-range-from:focus, #csv-row-range-to:focus {
                                border-color: #0d6efd;
                ...
                // On import
                $('#import-mapped-csv-btn').off('click').on('click', function() {
                    const mapping = {};
                    $('.csv-map-select').each(function() {
                        const colIdx = $(this).data('col');
                        const field = $(this).val();
                        if (field) mapping[colIdx] = field;
                    });
                    const from = parseInt($('#csv-row-range-from').val());
                    const to = parseInt($('#csv-row-range-to').val());
                    const name = $('#audience-name-csv-create').val();
                    const format = $('#csv-format-create').val();
                    ...
                    $('#csv-import-progress').show();
                    $('#csv-import-progress-text').text('Starting import...');
                    ...
                    let progressInterval = setInterval(function() {
                        $.get('/drift/audiences/import-progress', {
                            token: token
                        }, function(progressRes) {
                            console.log('Progress update:', progressRes); // Debug log
                            if (progressRes.current !== undefined && progressRes.total) {
                                ...
                                $('#csv-import-progress-text').text('Processing...');
                            }
                        }).fail(function(xhr) {
                            console.log('Progress request failed:', xhr); // Debug log
                    ...
                    $.ajax({
                        url: '/drift/audiences/process-csv',
                        method: 'POST',
                        ...
                        success: function(res) {
                            ...
                            let downloadHtml = '';
                            if (res.failed_csv) {
                                downloadHtml += `<a href="${res.failed_csv}" class="btn btn-warning btn-sm me-2" onclick="downloadCsv('${res.failed_csv}')">
                                    <i class="bi bi-download"></i> Download Failed Rows (${res.failed_count})
                                </a>`;
                            }
                            if (res.remaining_csv) {
                                downloadHtml += `<a href="${res.remaining_csv}" class="btn btn-info btn-sm" onclick="downloadCsv('${res.remaining_csv}')">
                                    <i class="bi bi-download"></i> Download Remaining Rows (${res.remaining_count})
                                </a>`;
                            }

                            if (downloadHtml) {
                                $('#csv-import-results').html(`
                                    <div class="alert alert-success mt-3">
                                        ...
                                        ${downloadHtml}
                                        <hr>
                                        <button type="button" class="btn btn-secondary btn-sm" onclick="$('#csvMappingModal').modal('hide'); $('#csv-import-results').html('');">
            ...
            function renderCsvPreview(header, rows) {
                let html = '<div id="csv-preview-table-container" style="max-height:350px;overflow-y:auto;">';
                html += '<table class="table table-bordered table-sm"><thead><tr>';
                html += '<th><input type="checkbox" id="csv-header-checkbox"></th>';
                html += '<th>S.No</th>';
                header.forEach(h => {
                    html += `<th>${h}</th>`;
                });
                ...
                rows.forEach((row, idx) => {
                    html +=
                        `<tr data-sno="${idx+1}"><td><input type="checkbox" class="csv-row-checkbox" data-row="${idx}" checked></td><td>${idx+1}</td>`;
                    header.forEach((_, colIdx) => {
                ...
                $('#csv-preview-section').html(html);
                // Header checkbox logic
                $('#csv-header-checkbox').off('change').on('change', function() {
                    const checked = $(this).is(':checked');
                    $('#csv-preview-section input.csv-row-checkbox').prop('checked', checked);
                });
            }

            function offerLeftoverCsvDownload(leftoverRows, header) {
                // Collect all unique keys from leftover rows (including 'reason')
                ...
                let csv = finalHeader.join(',') + '\n';
                leftoverRows.forEach(row => {
                    csv += finalHeader.map(h => (row[h] !== undefined ? row[h] : '')).join(',') + '\n';
                });
                ...
                link.href = url;
                link.download = 'leftover_subscribers.csv';
            ...
            $(document).off('click', '[id^=add-sequence-btn-]').on('click', '[id^=add-sequence-btn-]', function() {
                const $btn = $(this);
                ...
                axios.post('/drift/sequences', {
                    ...
                    .then(response => {
                        ...
                        // PATCH: Update nav buttons after DOM update
                        setTimeout(() => {
                            updateSequenceNavButtons(setId);
                        }, 0);
                        $btn.prop('disabled', false).find('.loading-spinner').remove();
                    })
                    .catch(error => {
                ...
                axios.get(`/drift/reports/${sequenceId}`)
                    .then(response => {
                        ...
                        emails.forEach((email, idx) => {
                            ...
                            let receiverEmail = 'Unknown';
                            let body = (typeof email === 'object' && email.body) ? email.body :
                                '';
                            if (category === 'sent') {
                                // Use sender/recipient fields if present, fallback to account_email/subscriber_email
                                senderEmail = (typeof email === 'object' && (email.sender ||
                                    email.account_email)) ? (email.sender || email
                                    .account_email) : 'Unknown';
                                receiverEmail = (typeof email === 'object' && (email
                                    .recipient || email.subscriber_email || email.email
                                )) ? (email.recipient || email.subscriber_email ||
                                    email.email) : (typeof email === 'string' ? email :
                                    'Unknown');
                                content += `
                        <div class="email-pair p-2 border-bottom text-muted" style="cursor:pointer;" data-idx="${idx}">
                            <span class="text-primary">${senderEmail}</span>
                            <span class="mx-2">→</span>
                            <span class="text-success">${receiverEmail}</span>
                        </div>
                        <div class="email-body-content" id="email-body-${idx}" style="display:none; margin-bottom:10px;">
                            ${body}
                        </div>
                    `;
                            } else {
                                senderEmail = (typeof email === 'object' && email
                                    .account_email) ? email.account_email : 'Unknown';
                                receiverEmail = (typeof email === 'object' && (email
                                    .subscriber_email || email.email)) ? (email
                                    .subscriber_email || email.email) : (typeof email ===
                        ...
                        <div class="email-body-content" id="email-body-${idx}" style="display:none; margin-bottom:10px;">
                            ${renderGmailStyleBody(body)}
                        </div>
                    `;
                            }
                        });
                        $('#replied-emails-content').html(content);
                        // Add click handler for toggling body
                        $('#replied-emails-content .email-pair').on('click', function() {
                            const idx = $(this).data('idx');
                            const email = emails[idx];
                            let body = (typeof email === 'object' && email.body) ? email.body :
                                '';
                            if (category === 'sent') {
                                // For sent, inject HTML as-is
                                $('#email-body-modal-content').html(body);
                            } else {
                                $('#email-body-modal-content').html(renderGmailStyleBody(body));
                            }
            ...
            $(document).off('click', '.report-category').on('click', '.report-category', function() {
                ...
                axios.get(`/drift/reports/${sequenceId}`)
                    .then(response => {
                        ...
                        emails.forEach((email, idx) => {
                            ...
                            const receiverEmail = (typeof email === 'object' && (email
                                ...
                                .subscriber_email || email.email) : (typeof email ===
                                'string' ? email : 'Unknown');
                            const body = (typeof email === 'object' && email.body) ? email
                                .body : '';
                            if (category === 'sent') {
                                content += `
                        <div class="email-pair p-2 border-bottom text-muted" style="cursor:pointer;" data-idx="${idx}">
                    ...
                    `;
                        ...
                        $('#replied-emails-content .email-pair').on('click', function() {
                            const idx = $(this).data('idx');
                            const body = emails[idx] && emails[idx].body ? emails[idx].body :
                                '';
                            $('#email-body-modal-content').html(renderGmailStyleBody(body));
                            $('#emailBodyModal').modal('show');
                        });
        ...
        // Add search input above the replied-emails-content div
        $(document).ready(function() {
            if ($('#replied-emails-search').length === 0) {
                $('#replied-emails-content').before(`
                    <div class="mb-2">
                        <input type="text" id="replied-emails-search" class="form-control" placeholder="Search emails...">
                    </div>
                `);
            }
            // Search/filter logic
            $(document).on('input', '#replied-emails-search', function() {
                const search = $(this).val().toLowerCase();
                let anyVisible = false;
                $('#replied-emails-content .email-pair').each(function() {
                    const idx = $(this).data('idx');
                    const text = $(this).text().toLowerCase();
                    const body = $(`#email-body-${idx}`).text().toLowerCase();
                    if (text.includes(search) || body.includes(search)) {
                        $(this).show();
                        $(`#email-body-${idx}`).show(); // Only show if previously toggled open
                        anyVisible = true;
                    } else {
                        $(this).hide();
                        $(`#email-body-${idx}`).hide();
                ...
                if (!anyVisible) {
                    if ($('#replied-emails-no-results').length === 0) {
                        $('#replied-emails-content').append(
                            '<div id="replied-emails-no-results" class="text-muted p-2">No results found.</div>'
                        );
                    }
                } else {
                    $('#replied-emails-no-results').remove();
                }
            });
        });

        // Patch: After adding a new sequence, update navigation buttons so only the last sequence shows Save, others show Next
        function updateSequenceNavButtons(setId) {
            const $tabs = $(`#sequenceTabs-${setId} li.nav-item`).not(`#add-sequence-btn-${setId}`);
            $tabs.each(function(index) {
                const tabIndex = index + 1;
                const isLast = index === $tabs.length - 1;
                const $tabPane = $(`#sequence-${setId}-${tabIndex}`);
                const $navContainer = $tabPane.find('.d-flex.gap-2.justify-content-center');
                if ($navContainer.length) {
                    $navContainer.empty();
                    $navContainer.append(`
                        <button type="button" class="btn btn-outline-secondary prev-sequence" data-sequence="${$tabPane.data('sequence')}" data-tab-index="${tabIndex}" data-set-id="${setId}" ${tabIndex === 1 ? 'disabled' : ''}>Previous</button>
                        ${isLast ? `<button type="button" class="btn btn-primary save-sequence-final" data-sequence="${$tabPane.data('sequence')}" data-tab-index="${tabIndex}" data-set-id="${setId}">Save</button>` : `<button type="button" class="btn btn-outline-secondary next-sequence" data-sequence="${$tabPane.data('sequence')}" data-tab-index="${tabIndex}" data-set-id="${setId}">Next</button>`}
                        <button type="button" class="btn btn-primary preview-email" data-sequence="${$tabPane.data('sequence')}">Preview</button>
                    `);
                }
            });
        }
        // Call updateSequenceNavButtons after adding a new sequence
        $(document).on('click', `#add-sequence-btn-${'${setId}'}`, function() {
            // ... existing add sequence logic ...
            updateSequenceNavButtons(activeSetId);
        });
        // Also call after any sequence is saved or navigated
        $(document).on('click', '.save-sequence-final, .next-sequence, .prev-sequence', function() {
            const setId = $(this).data('set-id');
            updateSequenceNavButtons(setId);
        });
        $(document).off('click', '.next-sequence').on('click', '.next-sequence', function() {
            const setId = $(this).data('set-id');
            const tabIndex = parseInt($(this).data('tab-index'));
            saveSequenceAndNavigate(setId, tabIndex, 'next');
            setTimeout(() => {
                const newTabIndex = tabIndex + 1;
                // Find the sequence data for the new tab (from JS state or DOM)
                let sequence = null;
                if (sequencesState[setId] && sequencesState[setId][newTabIndex]) {
                    sequence = sequencesState[setId][newTabIndex];
                } else {
                    // fallback: try to get from DOM (minimal)
                    sequence = {
                        sequence_id: $(
                                `#sequence-${setId}-${newTabIndex}-form input[name='sequence_id']`)
                            .val()
                    };
                }
                populateSequenceForm(setId, newTabIndex, sequence);
            }, 350);
        });
        $(document).off('click', '.prev-sequence').on('click', '.prev-sequence', function() {
            const setId = $(this).data('set-id');
            const tabIndex = parseInt($(this).data('tab-index'));
            saveSequenceAndNavigate(setId, tabIndex, 'prev');
            setTimeout(() => {
                const newTabIndex = tabIndex - 1;
                let sequence = null;
                if (sequencesState[setId] && sequencesState[setId][newTabIndex]) {
        ...
        // Patch: After saving a template, refresh the template select in the current tab
        $(document).on('hidden.bs.modal', '#editorModal', function() {
            // Find the currently active sequence tab
            const $activeTab = $('.tab-pane.drift-sequence-box.active');
            if ($activeTab.length) {
                const setId = $activeTab.data('set');
                const seqId = $activeTab.data('sequence');
                const tabIndex = $activeTab.attr('id') ? parseInt($activeTab.attr('id').split('-')[2]) : null;
                if (setId && tabIndex) {
                    const $templateSelect = $(`#sequence-template-${setId}-${tabIndex}`);
                    if ($templateSelect.length) {
                        initializeSelect2($templateSelect);
        ...
        // Gmail-style email body rendering for category modal
        function renderGmailStyleBody(body) {
            if (!body) return '<div class="gmail-body-empty"><em>No body available</em></div>';
            let isHtml = /<\s*html|<\s*body|<\s*div|<\s*p|<\s*br|<\s*table|<\s*span|<\s*a/i.test(body);
            let processed = body;
            if (!isHtml) {
                // Escape HTML
                processed = processed.replace(/[&<>]/g, function(tag) {
                    return {
                        '&': '&amp;',
                        '<': '&lt;',
                        '>': '&gt;'
                ...
                // Style quoted text
                processed = processed.replace(/(^|\n)(>.*)/g, function(_, br, quote) {
                    return br + '<span class="gmail-quote">' + quote + '</span>';
                });
                // Auto-link URLs
            ...
            return `
                <div class="gmail-body" style="background:#fff; border:1px solid #e0e0e0; border-radius:8px; padding:18px 24px; margin:8px 0; font-family:Roboto,Arial,sans-serif; font-size:15px; color:#222; max-height:400px; overflow:auto; box-shadow:0 1px 3px rgba(60,64,67,.08);">
                    <style>.gmail-quote{display:block;margin-left:16px;color:#6b6b6b;}</style>
                    ${processed}
                </div>
            `;
        }

        // Add the resetCreateAudienceModal function
        function resetCreateAudienceModal() {
            $('#audience-name-create').val('');
            $('#subscribers-input-create').val('');
            $('#subscriber-format-create').val('first-email');
            $('#audience-name-csv-create').val('');
        ...
        // Add autoDownloadLeftoverCsv function
        function autoDownloadLeftoverCsv(leftoverRows, header) {
            // Collect all unique keys from leftover rows (including 'reason')
            const allKeys = new Set(header);
            leftoverRows.forEach(row => {
                Object.keys(row).forEach(k => allKeys.add(k));
        ...
        // Add: Reset create audience modal when mapping modal is closed
        $('#csvMappingModal').off('hidden.bs.modal').on('hidden.bs.modal', function() {
            resetCreateAudienceModal();
        });
        $('#createAudienceModal').off('hidden.bs.modal').on('hidden.bs.modal', function() {
            resetCreateAudienceModal();
        });
        // Ensure resetCreateAudienceModal is called every time the create audience modal is opened
        $('#createAudienceModal').off('show.bs.modal').on('show.bs.modal', function() {
            resetCreateAudienceModal();
        });

        // Add a progress bar for backend import in the mapping modal
        if ($('#csv-import-progress-bar').length === 0) {
            $('#csv-mapping-section').append(`
                <div id="csv-import-progress-bar" class="my-2" style="display:none;">
                    <div class="progress">
        ...
        function startImportProgressPolling(token, total) {
            $('#csv-import-progress-bar').show();
            importProgressInterval = setInterval(function() {
                $.get('/drift/audiences/import-progress', {
                    token
                }, function(res) {
                    const percent = Math.round((res.current / res.total) * 100);
                    $('#csv-import-progress-bar .progress-bar')
                        .css('width', percent + '%')
                        .attr('aria-valuenow', percent)
                        .text(res.current + ' / ' + res.total);
                });
            }, 1000);
        }
        ...
        $(document).ready(function() {
            ...
            // Centralized action button handlers
            $('#drift-action-btn').on('click', function() {
                const activeTab = $('#setTabs .nav-link.active');
                const actionType = activeTab.attr('data-type');
                const actionText = activeTab.attr('data-text');

                console.log('Drift action button clicked:', actionType, actionText);

                if (actionType === 'manage') {
                    // Handle set management
                    console.log('Managing set:', activeTab.attr('data-token'));
                }
            });

            // Handle sequence action buttons dynamically
            $(document).on('click', '[id^="sequence-action-btn-"]', function() {
                const setId = $(this).attr('id').replace('sequence-action-btn-', '');
                const activeTab = $(`#sequenceTabs-${setId} .nav-link.active`);
                const actionType = activeTab.attr('data-type');
                const actionText = activeTab.attr('data-text');

                console.log('Sequence action button clicked:', actionType, actionText, 'for set:', setId);

                if (actionType === 'edit') {
                    // Handle sequence editing
                    console.log('Editing sequence:', activeTab.attr('data-token'));
                }
            });

            // Handle tab changes to update action buttons
            $('#setTabs').on('shown.bs.tab', '.nav-link', function(e) {
                const actionText = $(e.target).attr('data-text');
                const actionClass = $(e.target).attr('data-class');
                $('#drift-action-btn').text(actionText).removeClass().addClass('btn ' + actionClass);
            });

            // Handle sequence tab changes
            $(document).on('shown.bs.tab', '[id^="sequenceTabs-"] .nav-link', function(e) {
                const setId = $(e.target).closest('[id^="sequenceTabs-"]').attr('id').replace(
                    'sequenceTabs-', '');
                const actionText = $(e.target).attr('data-text');
                const actionClass = $(e.target).attr('data-class');
                $(`#sequence-action-btn-${setId}`).text(actionText).removeClass().addClass('btn ' +
                    actionClass);
            });

            // Set initial action button states
            setTimeout(() => {
                const activeSetTab = $('#setTabs .nav-link.active');
                if (activeSetTab.length) {
                    const actionText = activeSetTab.attr('data-text');
                    const actionClass = activeSetTab.attr('data-class');
                    $('#drift-action-btn').text(actionText).removeClass().addClass('btn ' + actionClass);
                }
            }, 1000);
        });
