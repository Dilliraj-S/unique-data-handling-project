...
class MasterFlowService
{
    ...
    public function SmtpUpdateMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        ");
            ...
            // ✅ Perform join to map SMTP and company IDs
            $results = DB::connection('central')->select("
            SELECT 
                t.li_company_id,
                t.li_company_name,
                s.lic_smtp,
    ...
    public function SmtpBaseMappingMethod(string $workflowName, array $input, array $output, array $metadata): array
    {
        ...
        try {
            ...
            $query = "SELECT $select FROM $tempTable t LEFT JOIN $supportTable s ON $on";
            $results = DB::connection('central')->select($query);
    ...
    //     Log::info("📥 Starting CompanyAboutMethod (SQL-Optimized, ID-preserving)", [
    //         'workflow' => $workflowName,
    //         'input_row_count' => count($input['rows'])
    //     ]);
    ...
    //         // ✅ Create temporary table (SMTP-style)
    ...
    //         // ✅ Insert rows into temp table (lowercased/trimmed)
    //         $maxPlaceholders = 10000;
    //         $columnCount = count($mappingHeaders);
    //         $batchSize = max(1, floor($maxPlaceholders / $columnCount));
    //         $insertBatch = [];
    ...
    //         foreach ($input['rows'] as $row) {
    //             // Build values in mappingHeaders order but from requiredHeaders
    //             $keyParts = array_map(function ($i) use ($row, $requiredHeaders) {
    //                 return strtolower(trim($row[$requiredHeaders[$i]] ?? ''));
    //             }, array_keys($mappingHeaders));
    //             $insertBatch[] = $keyParts;
    ...
    //         // ✅ Join with support table, selecting only necessary columns
    //         // If lic_company_about (TEXT) is in updateHeaders, it will be selected; otherwise it’s excluded.
    //         $on = implode(' AND ', array_map(fn($col) => "t.`$col` = s.`$col`", $mappingHeaders));
    //         $select = implode(', ', array_merge(
    //             array_map(fn($col) => "s.`$col`", $updateHeaders),
    //             array_map(fn($col) => "t.`$col`", $mappingHeaders)
    ...
    //         // ✅ Process input rows (SMTP-style flow)
    //         foreach ($input['rows'] as $row) {
    //             $metrics['total']++;
    //             $outputRow = $row; // preserve id, etc.

    //             // Mandatory skip (for CompanyAbout we require at least one of the keys; adapt to your rule)
    //             if ($mandatory && (empty(trim($row[$requiredHeaders] ?? '')) && empty(trim($row[$requiredHeaders] ?? '')))) {
    //                 $outputRow['workflow_status'] = 'skipped';
    //                 $outputRow['workflow_reason'] = "Missing or NULL `{$requiredHeaders}` and `{$requiredHeaders}`";
    //                 $metrics['skipped']++;
    //                 $outputRows[] = $outputRow;
    //                 continue;
    //             }
    ...
    //             // Valid if any update header has a non-null value
    //             $hasValidMatch = $match && !empty(array_filter(array_intersect_key($match, array_flip($updateHeaders)), fn($v) => $v !== null));

    //             if ($mandatory) {
    //                 if (!$hasValidMatch) {
    //                     $outputRow['workflow_status'] = 'rejected';
    //                     $outputRow['workflow_reason'] = "No valid Company About match found for {$mappingHeaders}";
    //                     $metrics['rejected']++;
    //                 } else {
    //                     foreach ($updateHeaders as $field) {
    //                         if (isset($match[$field])) {
    //                             // Note: This includes lic_company_about only if requested in updateHeaders
    //                             $outputRow[$field] = $match[$field];
    //                         }
    //                     }
    //                     $outputRow['workflow_status'] = 'processed';
    ...
    //                 if (!$hasValidMatch) {
    //                     $outputRow['workflow_status'] = 'pending';
    //                     $outputRow['workflow_reason'] = "Optional step: no valid match found for {$mappingHeaders}";
    //                     $metrics['pending']++;
    //                 } else {
    //                     foreach ($updateHeaders as $field) {
    //                         if (isset($match[$field])) {
    ...
    //         Log::info("✅ Completed CompanyAboutMethod", [
    //             'workflow' => $workflowName,
    //             'metrics' => $metrics
    //         ]);
    //     } catch (\Throwable $e) {
    //         Log::error("❌ Error in CompanyAboutMethod", [
    //             'process_id' => $processId,
    //             'error' => $e->getMessage()
    //         ]);
