                ...
                <div class="modal-body">
                    <form id="new-set-form">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="sequence-count" name="sequence_count" min="1"
                                value="1" required>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="confirm-delete-sequence-btn">
                        Delete
                        <span class="spinner-border spinner-border-sm loading-spinner" role="status" aria-hidden="true"
...
<script>
    ...
    $(document).ready(function() {
        ...
        function initializeSelect2(selector) {
            ...
            // Only fetch email accounts for From Email dropdown
            if ($select.attr('id') && $select.attr('id').startsWith('sequence-from-email-')) {
                $.ajax({
                    url: '/api/email-accounts',
                    method: 'GET',
                    success: function(data) {
                        emailAccountsData = data; // Store email accounts data
                        $select.empty().append('<option value="">Select Email Accounts</option>')
                            .append('<option value="__select_all__">-- Select All --</option>');
                        data.forEach(account => {
                            const displayText =
                                `${account.email} (${account.daily_send_limit})`;
                            $select.append(
                        ...
                        if (selected && Array.isArray(selected)) {
                            ...
                            console.log('Initialized Select2 for', $select.attr('id'),
                                'with selected:', selected);
                        }
                    },
                    error: function(xhr) {
                        console.error('Failed to fetch email accounts:', xhr.responseText);
                        $select.empty().append(
                            '<option value="">No email accounts available</option>');
                    }
                });
            } else {
                // Original logic for all other dropdowns (e.g., sequence-report-filters-, sequence-audience-)
                const selected = $select.data('selected');
                if (selected && Array.isArray(selected)) {
                    $select.val(selected).trigger('change.select2');
...
`);
        ...
        // Handle Configure Assignment button click
        $(document).on('click', '.configure-assignment', function() {
            ...
            $('#assignmentModal').data('set-id', setId).data('tab-index', tabIndex).data(
        ...
        `);
            ...
            <div id="modal-manual-assign-section">
                ...
                <p><strong>Selected From Emails:</strong> <span id="modal-from-emails-list">None</span></p>
        ...
        `);
            ...
            $('#assignmentModal').one('shown.bs.modal', function() {
                ...
                console.log('[DEBUG] Modal shown, audienceId from form:', audienceId, 'setId:',
                    setId, 'tabIndex:', tabIndex);
                if (!audienceId) {
                    console.warn('[DEBUG] No audienceId found for setId:', setId, 'tabIndex:',
                        tabIndex,
                        'Check if audience is selected or form is populated correctly.');
                }
                if ($('#modal-total-subscribers').length) {
                    console.log('modal-total-subscribers element exists in DOM');
                } else {
                    console.error('modal-total-subscribers element not found in DOM');
                }
        ...
        function updateModalAssignments(setId, tabIndex) {
            ...
            console.log('Updating modal assignments, fromEmails:', fromEmails);
            // Update Selected From Emails list with daily_send_limit
            const fromEmailsDisplay = fromEmails.map(email => {
        ...
        function updateModalWarnings(setId, tabIndex) {
            ...
            const totalSubscribers = parseInt($('#modal-total-subscribers').data('count') || $(
                '#modal-total-subscribers').text()) || 0;
            const fromEmails = $(`#sequence-from-email-${setId}-${tabIndex}`).val() || [];

            console.log('Updating warnings, mode:', mode, 'totalSubscribers:', totalSubscribers, 'fromEmails:',
                fromEmails);
            if (mode === 'batch_size') {
        ...
        // Update manual assignment inputs
        function updateManualAssignments(setId, tabIndex) {
            const $fromEmailSelect = $(`#sequence-from-email-${setId}-${tabIndex}`);
            const fromEmails = $fromEmailSelect.val() || [];
            const $assignmentsContainer = $(`#manual-assignments-${setId}-${tabIndex}`);
            const $fromEmailsList = $(`#from-emails-list-${setId}-${tabIndex}`);

            $fromEmailsList.text(fromEmails.join(', ') || 'None');
            $assignmentsContainer.empty();

            fromEmails.forEach(email => {
                $assignmentsContainer.append(`
            <div class="mb-2">
                <label for="manual-assign-${setId}-${tabIndex}-${email.replace(/[^a-zA-Z0-9]/g, '')}" class="form-label">${email}</label>
                <input type="number" class="form-control manual-assign-input" 
                       id="manual-assign-${setId}-${tabIndex}-${email.replace(/[^a-zA-Z0-9]/g, '')}" 
                       data-email="${email}" min="0" value="0">
            </div>
        `);
            });
        ...
        function updateAssignmentWarnings(setId, tabIndex) {
            ...
            if (mode === 'batch_size') {
                ...
                let totalAssigned = 0;
                $(`#manual-assignments-${setId}-${tabIndex} .manual-assign-input`).each(function() {
                    totalAssigned += parseInt($(this).val()) || 0;
                });
                const skipped = totalSubscribers - totalAssigned;
        ...
        $(document).on('submit', '[id^=sequence-][id$=-form]', function(e) {
            ...
            if (tabIndex == 1) {
                ...
                data.categories = $form.find(`#sequence-report-filters-${setId}-${tabIndex}`).val() || [];
            }
            ...
            axios.post('/drift/save-sequences', {
                ...
                .catch(error => {
                    ...
                    alert('Failed to save sequence: ' + (error.response?.data?.error ||
                        'Unknown error'));
                });
        });
        ...
        $('#save-all-sequences-btn').click(function() {
            ...
            $sequenceForms.each(function(index) {
                ...
                if (tabIndex === 1) {
                    ...
                    sequenceData.categories = $form.find(
            ...
            axios.post('/drift/save-sequences', {
                ...
                .then(response => {
                    ...
                    // Update sequence IDs if new
                    sequences.forEach((sequence, index) => {
                        const tabIndex = index + 1;
                        const newSequenceId = response.data.sequences?.[index]?.id;

                        if (newSequenceId && (!sequence.sequence_id || sequence.sequence_id
                                .startsWith('new-'))) {
                ...
                .catch(error => {
                    ...
                    alert(`Failed to save sequences: ${errorMsg}`);
                })
                .finally(() => {
    ...
    <i class="bi bi-trash"></i>
        ...
        $(document).on('click', '.send-immediately', function() {
            ...
            if (!$firstForm.length) {
                alert('First sequence form not found.');
                return;
            }
            ...
            $sequenceForms.each(function(index) {
                ...
                const sequenceData = {
                    ...
                    time_gap: formTabIndex === 1 ? parseInt($form.find(
                        `#sequence-time-gap-${setId}-${formTabIndex}`).val()) || null : null,
