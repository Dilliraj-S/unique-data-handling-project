...
class FormCtrl extends Controller
{
    ...
    private function processSmallMapping(array $config): JsonResponse
    {
        try {
            ...
            return ResponseHelper::moduleError('Query Execution Error', $e->getMessage(), 500);
        }
    }
    ...
    private function buildMappingQuery(array $config): string
    {
        ...
        // Build select fields: all main as main_*, selected mapped as mapped_*
        $selectFields = [];
        foreach ($mainFields as $field) {
            $selectFields[] = "m.`{$field}` AS `main_{$field}`";
        }
        foreach ($pairs as $pair) {
            $mapped = $pair['mapped'];
            $selectFields[] = "mp.`{$mapped}` AS `mapped_{$mapped}`";
        }
        $selectClause = implode(', ', $selectFields);
        
        // Build join conditions (assuming same field names for mapping)
        $joinConditions = [];
        foreach ($pairs as $pair) {
            $mainF = $pair['main'];
            $mappedF = $pair['mapped'];
        ...
        // Determine base FROM and JOIN based on result_type_1
        $fromClause = '';
        $baseWhere = '';
        switch ($config['result_type_1']) {
            case 'all_mapped':
                $fromClause = "FROM {$mappedTable} mp LEFT JOIN {$mainTable} m ON {$onClause}";
                break;
            case 'common':
            ...
            case 'not_in_main':
                $fromClause = "FROM {$mappedTable} mp LEFT JOIN {$mainTable} m ON {$onClause}";
                // Filter for no match in main
                $nullConditions = [];
                foreach ($pairs as $pair) {
                    $mainF = $pair['main'];
                    $nullConditions[] = "m.`{$mainF}` IS NULL";
                }
                $baseWhere = ' WHERE (' . implode(' AND ', $nullConditions) . ')';
                break;
            default:
                $fromClause = "FROM {$mappedTable} mp LEFT JOIN {$mainTable} m ON {$onClause}";
        }
        
        // Build secondary conditions based on result_type_2
        $secondaryWhere = '';
        switch ($config['result_type_2']) {
            case 'all_matched':
                // Filter to only rows where there is a match (main fields not null)
                $matchConditions = [];
                foreach ($pairs as $pair) {
                    $mainF = $pair['main'];
            ...
            case 'all_empty':
                // Rows where AT LEAST ONE mapped field is empty (NULL, '', or whitespace) in EITHER table
                $emptyConditions = [];
            ...
            case 'all_non_empty':
                // Rows where ALL mapped fields are non-empty in BOTH tables
                $nonEmptyConditions = [];
                foreach ($pairs as $pair) {
                    $mainF = $pair['main'];
                    $mappedF = $pair['mapped'];
        ...
        // Full query
        $query = "SELECT {$selectClause} {$fromClause} {$whereClause}";
